...
MatrixXd GradientDescent(MatrixXd para_guess, MatrixXd deflection, MatrixXd ym, double area_1, double area_2, double len_1, double len_2, double t_total, double ForceMax) {
	...
	while (counter < maxiter) {
		...
		if (counter == 0) {
			y_est = function_y(para_est, area_1, area_2, len_1, len_2, t_total, ForceMax);
			d = ym - y_est;
		}
		else {
			y_est = y_est_gd;
		...
		//cout << "dp: \n" << dp;
		para_gd = para_est + dp;
		y_est_gd= function_y(para_gd, area_1, area_2, len_1, len_2, t_total, ForceMax);
